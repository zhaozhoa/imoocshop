<template>
  <div>
    ...
    <Modal :mdShow="modalConfirm" @close="closeModal">
      <p slot="message">你确定要删除这件商品吗？</p>
      <div slot="btnGroup" >
        <a href="javascript:;" class="btn btn--m" @click="delCart">嗯</a>
       <a href="javascript:;" class="btn btn--m" @click="modalConfirm = false">再想想看</a>
      </div>
    </Modal>
    <!-- 商品数量控制模态框 -->
    <Modal :mdShow="productNumCont" @close="closeModal">
      <p slot="message">不能再减少了</p>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="productNumCont = false">知道啦</a>
      </div>
    </Modal>
    <NavFooter/>
  </div>
...
export default{
  ...
  methods: {
    ...
    // 商品数量的修改
    editCart (option, item) {
      if (option === 'minu') {
        if (item.productNum <= 1) {
          this.productNumCont = true
          return
        }
        item.productNum--
      } else if (option === 'add') {
      ...
      this.Axios.post('users/editGoodsNum', {
